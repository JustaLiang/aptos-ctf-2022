module ctfmovement::hello_move {
    ...
    #[test(dev=@0x11)]
    fun test_catch_the_flag(dev: &signer) acquires Challenge {
        use std::event;
        let dev_addr = signer::address_of(dev);
        account::create_account_for_test(dev_addr);
        init_challenge(dev);
        hash(dev, b"good");
        discrete_log(dev, 3123592912467026955);
        add(dev, 2, 0);
        get_flag(dev);
        let challenge = borrow_global<Challenge>(dev_addr);
        assert!(
            event::counter(&challenge.flag_event_handle) == 1,
            101,
        );
    }
}
